...
#define PF_MALLOC_ADDRESS SOC_EXTRAM_DATA_LOW
//#define PF_MEM_POOL_SIZE (SOC_EXTRAM_DATA_HIGH - SOC_EXTRAM_DATA_LOW)

/* CUSTOM: Make the memory pool bigger if you want. */
#undef PFM_MEM_POOL_SIZE
#ifndef PF_MEM_POOL_SIZE
//#define PF_MEM_POOL_SIZE (0x100000)
#define PF_MEM_POOL_SIZE (2048 * 1024)
#endif
...
void dllAddNodeToHead(DoublyLinkedList *ListPtr,
                      DoublyLinkedListNode *NewNodePtr) {
  dllInsertNodeBefore(NewNodePtr, ListPtr->dll_First);
}

void dllAddNodeToTail(DoublyLinkedList *ListPtr,
                      DoublyLinkedListNode *NewNodePtr) {
...
#define dllIsListEmpty(l)                                                      \
  ((l)->dll_First == ((DoublyLinkedListNode *)&((l)->dll_Null)))
...
void pfInitMemoryAllocator(void) {
  printf("%x %d\n", PF_MALLOC_ADDRESS, PF_MEM_POOL_SIZE);
  pfInitMemBlock(PF_MALLOC_ADDRESS, PF_MEM_POOL_SIZE);
}
#else /* PF_NO_MALLOC */

int not_an_empty_file; /* Stops nasty compiler warnings when PF_NO_MALLOC not
                          defined. */
