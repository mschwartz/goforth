...
{
  if (ffFindC("(QUIT)", &gQuitP_XT) == 0)
    goto nofind;
  if (ffFindC("NUMBER?", &gNumberQ_XT) == 0)
    goto nofind;
  if (ffFindC("ACCEPT", &gAcceptP_XT) == 0)
    goto nofind;
...
#include "pfcompfp.h"
  ...
  if (CompileCustomFunctions() < 0)
    goto error; /* Call custom 'C' call builder. */
...
ThrowCode ffSemiColon(void)
{
  ...
  if ((gDepthAtColon != DATA_STACK_DEPTH) && (gDepthAtColon != DEPTH_AT_COLON_INVALID)) /* Ignore if no ':' */
  {
...
ThrowCode ffIncludeFile(FileStream* InputFile)
{
  ...
  if (exception) {
    ...
    MSG("INCLUDE error on line #");
    ffDot(gCurrentTask->td_LineNumber);
    MSG(", level = ");
    ffDot(gIncludeIndex);
    EMIT_CR
    ...
    EMIT_CR;
    for (i = 0; i < (gCurrentTask->td_IN - 1); i++)
      EMIT('^');
    EMIT_CR;
  }
...
cell_t ffRefill(void)
{
  ...
  if (gCurrentTask->td_InputStream == PF_STDIN) {
    ...
    Num = readLineFromStream(gCurrentTask->td_SourcePtr, TIB_SIZE, gCurrentTask->td_InputStream);
    if (Num == EOF) {
      Result = 0;
      Num = 0;
    }
