...
The chunk IDs, sizes, and data values are all Big Endian in conformance with
the IFF standard. The dictionaries may be big or little endian. 'FORM' size
'P4TH'
-  Form Identifier
...
/***************************************************************/
static cell_t WriteChunkToFile(FileStream *fid, cell_t ID, char *Data,
                               int32_t NumBytes) {
  cell_t numw;
  cell_t EvenNumW;
  ...
  if (Write32ToFile(fid, (uint32_t)ID) < 0)
    goto error;
  assert(EvenNumW <= UINT32_MAX);
  if (Write32ToFile(fid, (uint32_t)EvenNumW) < 0)
    goto error;
...
static void convertDictionaryInfoWrite(DictionaryInfoChunk *sd) {
...
static void convertDictionaryInfoRead(DictionaryInfoChunk *sd) {
...
cell_t ffSaveForth(const char *FileName, ExecToken EntryPoint, cell_t NameSize,
  ...
  /* Write FORM Header ---------------------------- */
  if (Write32ToFile(fid, ID_FORM) < 0)
    goto error;
  if (Write32ToFile(fid, 0) < 0)
    goto error;
  if (Write32ToFile(fid, ID_P4TH) < 0)
    goto error;
  ...
  relativeCodePtr =
      ABS_TO_CODEREL(gCurrentDictionary->dic_CodePtr.Byte); /* 940225 */
  SD.sd_RelCodePtr = relativeCodePtr;
  SD.sd_UserStackSize = sizeof(cell_t) * (gCurrentTask->td_StackBase -
                                          gCurrentTask->td_StackLimit);
  SD.sd_ReturnStackSize = sizeof(cell_t) * (gCurrentTask->td_ReturnBase -
                                            gCurrentTask->td_ReturnLimit);
  ...
  if (WriteChunkToFile(fid, ID_P4DI, (char *)&SD, sizeof(DictionaryInfoChunk)) <
      0)
    goto error;

  /* Write Name Fields if NameSize non-zero ------- */
  if (NameSize > 0) {
    if (WriteChunkToFile(fid, ID_P4NM, (char *)NAME_BASE, NameChunkSize) < 0)
      goto error;
  }

  /* Write Code Fields ---------------------------- */
  if (WriteChunkToFile(fid, ID_P4CD, (char *)CODE_BASE, CodeChunkSize) < 0)
    goto error;

  FormSize = (uint32_t)sdTellFile(fid) - 8;
  sdSeekFile(fid, 4, PF_SEEK_SET);
  if (Write32ToFile(fid, FormSize) < 0)
    goto error;
...
/***************************************************************/
PForthDictionary pfLoadDictionary(const char *FileName,
  ...
  /* Read FORM, Size, ID */
  if (Read32FromFile(fid, &ChunkID) < 0)
    goto read_error;
  if (ChunkID != ID_FORM) {
  ...
  if (Read32FromFile(fid, &FormSize) < 0)
    goto read_error;
  BytesLeft = FormSize;
  ...
  while (BytesLeft > 0) {
    if (Read32FromFile(fid, &ChunkID) < 0)
      goto read_error;
    if (Read32FromFile(fid, &ChunkSize) < 0)
      goto read_error;
    BytesLeft -= 8;
    ...
    switch (ChunkID) {
      case ID_P4DI:
        ...
        if (!gVarQuiet) {
          MSG("pForth loading dictionary from file ");
          MSG(FileName);
          EMIT_CR;
...
/* Check for compatible float size. */
        ...
        if (sd->sd_NameSize > 0) {
          gVarContext =
              NAMEREL_TO_ABS(sd->sd_RelContext); /* Restore context. */
          gCurrentDictionary->dic_HeaderPtr =
              (ucell_t)(uint8_t *)NAMEREL_TO_ABS(sd->sd_RelHeaderPtr);
        }
        else {
          gVarContext = 0;
        ...
        gCurrentDictionary->dic_CodePtr.Byte =
            (uint8_t *)CODEREL_TO_ABS(sd->sd_RelCodePtr);
        gNumPrimitives =
            sd->sd_NumPrimitives; /* Must match compiled dictionary. */
                                  /* Pass EntryPoint back to caller. */
        if (EntryPointPtr != NULL)
          *EntryPointPtr = sd->sd_EntryPoint;
        pfFreeMem(sd);
        break;
...
PForthDictionary pfLoadStaticDictionary(void) {
  ...
  if (IF_LITTLE_ENDIAN != IsHostLittleEndian()) {
    MSG((IF_LITTLE_ENDIAN ? "Little Endian Dictionary on "
                          : "Big Endian Dictionary on "));
    MSG((IsHostLittleEndian() ? "Little Endian CPU" : "Big Endian CPU"));
    EMIT_CR;
  }
...
#define PF_EXTRA_CODE (40000)
  ...
  pfCopyMemory((uint8_t *)dic->dic_HeaderBase, MinDicNames,
               sizeof(MinDicNames));
